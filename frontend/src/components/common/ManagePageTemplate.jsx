      ...
      if (
        ...
        setError(
          err.response?.data?.message ||
            `เกิดข้อผิดพลาดในการลบ${popupTitlePrefix}`
        );
      }
    } finally {
      setLoading(false);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          ...
          {loading && items.length === 0 ? (
            <div className="flex items-center justify-center h-24">
              ...
              <p className="ml-4 text-gray-700">
                กำลังโหลด{popupTitlePrefix}...
              </p>
            </div>
          ) : items.length === 0 ? (
            <p className="text-gray-600 text-center">
              ยังไม่มี{popupTitlePrefix}
            </p>
          ) : (
                ...
                <thead className="bg-stromboli-100">
                  <tr>
                    {tableHeaders.map((header, index) => (
                      <th
                        key={index}
                        className="py-3 px-4 text-left text-xs font-semibold text-black uppercase tracking-wider"
                      >
                        {header.label}
                      </th>
                    ))}
          ...
          <Popup
            ...
            title={
              editingItem
                ? `แก้ไข${popupTitlePrefix}`
                : `เพิ่ม${popupTitlePrefix}ใหม่`
            }
          >
