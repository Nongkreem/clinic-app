...
exports.getAvailableSlotsByDateAndService = async (req, res) => {
  ...
  try {
    const slots = await DoctorSchedule.getAggregatedAvailableSlots(
      scheduleDate,
      parsedServiceId
    );
...
exports.bookNewAppointment = async (req, res) => {
  ...
  try {
    ...
    if (!slotRows.length) {
      return res.status(404).json({ message: "ไม่พบ slot ที่เลือก" });
    }

    const { schedule_date, slot_start } = slotRows[0];
    const now = new Date();
    const selectedSlotTime = new Date(`${schedule_date}T${slot_start}`);
    const diffHours = (selectedSlotTime - now) / (1000 * 60 * 60);

    if (diffHours < 24) {
      return res.status(400).json({
        success: false,
        message: "ไม่สามารถจอง slot ที่เหลือน้อยกว่า 24 ชั่วโมงได้",
      });
    }
    
    // ผ่านแล้ว -> สร้างการจอง
    ...
    res
      .status(500)
      .json({ success: false, message: "เกิดข้อผิดพลาดในการสร้างนัดหมาย" });
  }
};
...
exports.getPatientAppointments = async (req, res) => {
  ...
  try {
    ...
    if (appointments.length === 0) {
      console.log(
        "[DEBUG] getPatientAppointments - No appointments found for this patient."
      );
    }
...
exports.cancelPatientAppointment = async (req, res) => {
  ...
  if (!patient_id) {
    return res
      .status(401)
      .json({ message: "ไม่ได้รับอนุญาต: ไม่พบข้อมูลผู้ป่วย" });
  }
    ...
    if (result && result.newCount !== undefined) {
      ...
      if (result.isBlacklisted) {
        return res.status(403).json({
          message:
            "คุณถูกระงับการใช้งานเนื่องจากยกเลิกนัดเกิน 3 ครั้ง โปรดติดต่อเจ้าหน้าที่เพื่อปลดล็อก",
        });
      }
  ...
  try {
    ...
    if (success) {
      ...
      res
        ...
        .json({
          message:
            "ไม่สามารถยืนยันเข้ารับบริการได้ (อาจยังไม่ได้รับการอนุมัติ)",
        });
    }
  } catch (error) {
...
exports.getApprovedCheckedInForService = async (req, res) => {
  try {
    ...
    if (!serviceId)
      return res.status(400).json({ message: "ต้องระบุ serviceId" });

    const parsedServiceId = parseInt(serviceId, 10);
    if (isNaN(parsedServiceId))
      return res.status(400).json({ message: "serviceId ไม่ถูกต้อง" });

    const rows = await Appointment.getApprovedCheckedInAppointments(
      parsedServiceId
    );
